...
async function fetchCandleData(symbol, timeframe) {
  ...
  async function preLoadHistoryCandles(symbol, timeframe) {
    ...
    try {
      ...
      const data = await response.json();
      return data.map(candle => ({
        time: candle.timestamp / 1000, // Assuming the API returns timestamps in milliseconds
        open: candle.open,
        high: candle.high,
        low: candle.low,
        close: candle.close,
        // Include volume or other relevant data if needed
      }));
    } catch (error) {
  ...
  export { fetchCandleData, fetchAllLineData, preLoadHistoryCandles };
